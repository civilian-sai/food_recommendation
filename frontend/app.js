$(document).ready(function () {
  ...
  var getRecordsButton = $(
    '<button class="btn btn-info btn-records">Get Records</button>'
  );
  ...
  function displayRecommendations(recipes) {
    ...
    for (var i = 0; i < recipes.length; i++) {
      ...
      var defaultImageUrl =
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRcOt2m-NOqLUy6nzH9fFoHiXXzntfJ23bVKQ&s";
      ...
      // Remove first 2 characters and last character before splitting
      var cleanedIngredients = recipe.RecipeIngredientParts.slice(2, -1);
      // Split the cleaned string into an array
      var ingredientsArray = cleanedIngredients.split(", ");

      // Add ingredients list to card body
      var ingredientsList = $("<ul></ul>");
      ingredientsArray.forEach(function (ingredient) {
        var ingredientItem = $("<li>" + ingredient + "</li>");
        ingredientItem.appendTo(ingredientsList);
      });
      ingredientsList.appendTo(cardBody);
    ...
    var saveAllButton = $(
      '<button class="btn btn-success">Save All Recipes</button>'
    );
  ...
  function showRecipeDetails(recipe) {
    // You can adjust this function according to how you want to display detailed recipe information
    // For demonstration, let's alert the details
    alert(
      "Recipe Details:\n" +
        "Name: " +
        recipe.Name +
  ...
  function records(userEmail) {
    $.ajax({
      type: "POST",
      url: "http://127.0.0.1:3000/records",
  ...
  function saveRecipe(userEmail, recipeName) {
    ...
    $.ajax({
      type: "POST",
      url: "http://127.0.0.1:3000/saveRecipe",
